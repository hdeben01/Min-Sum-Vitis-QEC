...
// ==============================================================
`timescale 1ns/1ps
module compute_row_operations_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_L_DATA_1           = 7'h14,
    ADDR_L_CTRL             = 7'h18,
    ADDR_NON_ZERO_DATA_0    = 7'h1c,
    ADDR_NON_ZERO_DATA_1    = 7'h20,
    ADDR_NON_ZERO_CTRL      = 7'h24,
    ADDR_SYNDROME_DATA_0    = 7'h28,
    ADDR_SYNDROME_DATA_1    = 7'h2c,
    ADDR_SYNDROME_CTRL      = 7'h30,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NON_ZERO_DATA_0: begin
                    rdata <= int_non_zero[31:0];
                end
                ADDR_NON_ZERO_DATA_1: begin
                    rdata <= int_non_zero[63:32];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_NON_ZERO_DATA_0)
            int_non_zero[31:0] <= (WDATA[31:0] & wmask) | (int_non_zero[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_non_zero[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NON_ZERO_DATA_1)
            int_non_zero[63:32] <= (WDATA[31:0] & wmask) | (int_non_zero[63:32] & ~wmask);
    end
end
