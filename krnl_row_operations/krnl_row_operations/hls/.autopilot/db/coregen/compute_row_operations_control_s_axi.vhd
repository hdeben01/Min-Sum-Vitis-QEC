...
entity compute_row_operations_control_s_axi is
generic (
...
port (
    ...
    non_zero              :out  STD_LOGIC_VECTOR(63 downto 0);
...
end entity compute_row_operations_control_s_axi;
...
architecture behave of compute_row_operations_control_s_axi is
    ...
    constant ADDR_NON_ZERO_DATA_0    : INTEGER := 16#1c#;
    constant ADDR_NON_ZERO_DATA_1    : INTEGER := 16#20#;
    constant ADDR_NON_ZERO_CTRL      : INTEGER := 16#24#;
    constant ADDR_SYNDROME_DATA_0    : INTEGER := 16#28#;
    constant ADDR_SYNDROME_DATA_1    : INTEGER := 16#2c#;
    constant ADDR_SYNDROME_CTRL      : INTEGER := 16#30#;
    ...
    signal int_non_zero        : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NON_ZERO_DATA_0 =>
                        rdata_data <= RESIZE(int_non_zero(31 downto 0), 32);
                    when ADDR_NON_ZERO_DATA_1 =>
                        rdata_data <= RESIZE(int_non_zero(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    ...
    non_zero             <= STD_LOGIC_VECTOR(int_non_zero);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_non_zero(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NON_ZERO_DATA_0) then
                    int_non_zero(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_non_zero(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_non_zero(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NON_ZERO_DATA_1) then
                    int_non_zero(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_non_zero(63 downto 32));
                end if;
            end if;
        end if;
    end process;
